...
def dsigmoid(x):
    ...
    :param x: array-like shape(n_sample, n_feature) activation
...
class MLP:
    ...
    def compute_loss(self, X, y):
        ...
        Y = onehot(y)
        epi = 1e-40
        data_loss = -np.sum(Y*np.log(probs+epi)+(1-Y)*np.log(1-probs+epi))
        # Add regularization term to loss
        for i in range(0, len(self.hidden_layer_size)):
            data_loss += self.reg_lambda* np.sum(np.power(self.weights[i], 2))/2.0
    ...
    def forward(self, X):
        ...
        self.layers[self.n_layers+1] = self.output_layer(np.dot(self.layers[self.n_layers], self.weights[self.n_layers]) + self.bias[self.n_layers])
    ...
    def backward(self, X, y):
        ...
        for i in range(len(self.hidden_layer_size), 0, -1):
            self.deltas[i-1] = \
                np.dot(self.deltas[i], self.weights[i].T) * self.activation_dfunc(self.layers[i])

        # TODO update weights
        for i in range(len(self.hidden_layer_size), -1, -1):
            dw = np.dot(self.layers[i].T, self.deltas[i])
            dw += self.reg_lambda * self.weights[i]
            dw = self.lr * dw
    ...
    def score(self, X, y):
        ...
        probs = self.forward(X)
        # compute accuracy
        acc = np.sum((probs.argmax(axis=1) == np.ravel(y)) * 1.0 / n_samples)
        return acc
